-- Auto-copied from docs/DB_SCHEMA.sql for server-side application
...
CREATE TABLE clauses (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  document_id UUID REFERENCES documents(id) ON DELETE CASCADE,
  type TEXT, -- e.g., termination, confidentiality, indemnity
  text TEXT NOT NULL,
  page INT,
...
CREATE TABLE audit_logs (
  ...
  firm_id UUID REFERENCES firms(id) ON DELETE CASCADE,
  user_id UUID REFERENCES users(id),
  action TEXT NOT NULL, -- upload, view, delete, chat, admin_action
  resource_type TEXT NOT NULL, -- document, matter, client, user
  resource_id UUID,
  metadata JSONB,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
